"""Same-game call-graph propagation: CoD4X <- CoD4-Mac.
...
Function SIZE is deliberately never used as a filter (cross-compiler).
    ...
    python re/cod4x_propagate.py [min_support]
"""
...
def propagate(seed, verbose=True, min_support=None):
    ...
    tg, t_callees, t_callers = build("cod4x_funcs.json")
    rg, r_callees, r_callers = build("cod4_funcs.json")

    M = dict(seed)
    used_ref = set(M.values())

    # a reference name that is not unique cannot identify anything
    ref_clean = {}
...
if __name__ == "__main__":
    ...
    json.dump(rows, open(os.path.join(RE_DIR, "cod4x_propagated.json"),
                         "w", encoding="utf-8"))
    print("\nsample:")
    for r in rows[:20]:
