"""Baseline survey of any IDB pair: segments, function counts, naming coverage.

    python re/baseline2.py cod4x cod4
"""
import sys, os
...
CODE = r'''
def main():
    import idautils, ida_funcs, ida_segment, ida_name
    segs = []
...
for key in sys.argv[1:]:
    port = PORTS.get(key, None)
    if port is None:
        port = int(key)
    r = pyeval(port, CODE)
    if isinstance(r, str):
        print(key, "ERROR:", r[:500]); continue
    ...
    print("  segments:")
    for s in r["segs"]:
        print(f"    {s['name']:12} 0x{s['start']:x}-0x{s['end']:x}  "
              f"{s['size']/1048576:8.2f} MB  exec={s['exec']}")
