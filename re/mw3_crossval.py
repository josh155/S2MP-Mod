"""K-fold cross-validation of the MW3 anchor pass against call-graph propagation.
...
Writes re/mw3_contradictions.json (target eas to exclude from the apply).

    python re/mw3_crossval.py [folds] [repeats]
"""
import json, os, sys, random
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from mw3_common import RE_DIR, load
from mw3_propagate import propagate
...
for ms in (2, 3):
    ...
    per_tier = _c.defaultdict(lambda: [0, 0])
...
json.dump(sorted(all_contra.values(), key=lambda d: d["tgt"]),
          open(os.path.join(RE_DIR, "mw3_contradictions.json"),
               "w", encoding="utf-8"))
